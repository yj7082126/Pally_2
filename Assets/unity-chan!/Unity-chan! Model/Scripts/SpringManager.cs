...
namespace UnityChan
{
	public class SpringManager : MonoBehaviour
	{
		...
		private void UpdateParameters ()
		{
            ...
            var start = stiffnessCurve.keys[0].time;
            var end = stiffnessCurve.keys[stiffnessCurve.length - 1].time;
            ...
            for (int i = 0; i < springBones.Length; i++)
            {
                ...
                if (!springBones[i].isUseEachBoneForceSettings)
                {
                    ...
                    SpringBone prop = (SpringBone)(springBones[i]);
                    prop.stiffnessForce =  stiffnessForce * scale;
                }
            }
           ...
           // UpdateParameter ("dragForce", dragForce, dragCurve);
            ...
            end = dragCurve.keys[dragCurve.length - 1].time;
            //var step	= (end - start) / (springBones.Length - 1);
            //var prop = springBones[0].GetType().GetField(fieldName, System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
            ...
            for (int i = 0; i < springBones.Length; i++)
            {
                ...
                if (!springBones[i].isUseEachBoneForceSettings)
                {
                    ...
                    SpringBone prop = (SpringBone)(springBones[i]);
                    prop.dragForce = dragForce * scale;
                }
            }
        }
